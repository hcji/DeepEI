...
def build_RI_model_descriptor(morgan, cdkdes, RI, descriptor, save_name):
    ...
    history = model.fit(X_train, Y_train, epochs=100, callbacks=[earlyStopping, mcp_save, reduce_lr_loss], batch_size=1024, validation_split=0.11)
    ...
    return {'Y_test': Y_test, 'Y_predict': Y_predict, 'r2': r2, 'mae': mae}
...
def build_RI_model_CNN(smiles, RI, method, save_name):
    ...
    opt = optimizers.Adam(lr=0.001)
    model.compile(optimizer=opt, loss='mse', metrics=[metrics.mae])
    history = model.fit(X_train, Y_train, epochs=100, callbacks=[earlyStopping, mcp_save, reduce_lr_loss], validation_split=0.11)
